...
def plot4DC(x,y,t,u,name):
    X1,X2= torch.meshgrid(x,y)
    # Create the animation
    fig, ax = plt.subplots()
    contour = ax.contourf(X1, X2,u[:,:,0] ,50,cmap='rainbow')
    fig.canvas.draw()
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ...
    def update(i):
        ax.clear()
        ax.contourf(X1, X2,u[:,:,i] ,50, cmap='rainbow')
    ani = FuncAnimation(fig, update, frames=len(t), interval=1,blit=False)
    ani.save(name+'_4d_contourf.gif', writer='pillow')
...
low_bdry = xyt_test[0]
up_bdry = xyt_test[-1]
#plot4DC(xy[:,0],xy[:,1],t,uv_real,'analytic')
...
# Plot result
plt.semilogy(range(epoch),losses, label='Total Loss')
plt.semilogy(range(epoch),losses_pdes, label='PDE Residuals', linestyle='--')
plt.semilogy(range(epoch),losses_bcs, label='BC Residuals', linestyle='--')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Loss and PDE Residuals Over Epochs')
plt.legend()
plt.savefig('Residual loss per epoch.png')
...
uv_0_err = uv_0_arr - uv_test_arr
uv_1_err = uv_1_arr - uv_test_arr
#plot4DS(X[:,0,0],Y[0,:,0],T[0,0,:],uv_0)
#plot4DS(X[:,0,0],Y[0,:,0],T[0,0,:],uv_1)
#plot4DS(X[:,0,0],Y[0,:,0],T[0,0,:],uv_analytic)
plot4DC(X[:,0,0],Y[0,:,0],T[0,0,:],uv_0_err,'uv_0_err')
plot4DC(X[:,0,0],Y[0,:,0],T[0,0,:],uv_0_err,'uv_1_err')
